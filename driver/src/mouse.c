...
NTSTATUS MouseInit(VOID) {
    ...
    return (gMouseObject.mouse_device && gMouseObject.service_callback) ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
}

VOID MouseMove(LONG delta_x, LONG delta_y) {
    if (!gMouseObject.use_mouse) return;
    ...
    gMouseObject.service_callback(gMouseObject.mouse_device, &mid, (PMOUSE_INPUT_DATA)&mid + 1, &input_data);
    KeLowerIrql(irql);
}