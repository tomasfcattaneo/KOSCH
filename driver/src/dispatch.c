...
static NTSTATUS handle_get_module_base(PVOID buffer, ULONG size)
{
    ...
    ULONG len = 0;
    NTSTATUS status = ZwQuerySystemInformation(11, NULL, 0, &len);
    if (status != STATUS_INFO_LENGTH_MISMATCH || len == 0) {
        write_response(buffer, 1, 0);
        return STATUS_SUCCESS;
    }

    RTL_PROCESS_MODULES_K *mods = (RTL_PROCESS_MODULES_K *)ExAllocatePool2(POOL_FLAG_NON_PAGED, len, POOL_TAG);
    if (!mods) {
        write_response(buffer, 1, 0);
        return STATUS_SUCCESS;
    }

    status = ZwQuerySystemInformation(11, mods, len, &len);
    ...
    for (ULONG i = 0; i < mods->NumberOfModules; i++) {
        RTL_PROCESS_MODULE_INFORMATION_K *mod = &mods->Modules[i];
        char *name = (char*)mod->FullPathName + mod->OffsetToFileName;

        UNICODE_STRING us;
        RtlInitUnicodeString(&us, req->module_name);
        ANSI_STRING as;
        NTSTATUS conv_status = RtlUnicodeStringToAnsiString(&as, &us, TRUE);
        if (!NT_SUCCESS(conv_status)) continue;

        if (strcmp(as.Buffer, name) == 0) {
            result = (UINT64)mod->ImageBase;
